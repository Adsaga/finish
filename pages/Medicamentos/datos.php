<?php
	...
	if(!$conexion)
	{
		...
		if ($ncapa==3){//CAPA 3 PARA GUARDAR DATOS
			...
			echo $lote=$_REQUEST['lote'];
			echo $nombre=$_REQUEST['nombre']; 
			echo $fecha=$_REQUEST['fecha'];
			echo $laboratorio=$_REQUEST['lab'];
			echo $tipo=$_REQUEST['tipo'];
			echo $clasificacion=$_REQUEST['clasif'];
			echo $aplicacion=$_REQUEST['aplic'];
			echo $cantidad=$_REQUEST['cant'];
			echo $unidad=$_REQUEST['udm'];
					
		
 				# Contenido del archivo
		       
				$variable1="insert into medicamentos"; 
				$campos=" (Id_Clave,Numero_Lote,Nombre_Medicamento,Caducidad,Cantidad,Id_LaboratorioR,Id_ClasificacionR,Id_AplicacionR,Id_UnidadMedidaR,Id_TipoR)";
				echo $valores=" values('$clave','$lote','$nombre','$fecha',$cantidad,$laboratorio,$clasificacion,$aplicacion,$unidad,$tipo)";
				...
				if(!$rs){
						...
						$_SESSION['tipo']=$tipo;
						$_SESSION['clasif']=$clasificacion;
						$_SESSION['aplic']=$aplicacion;
