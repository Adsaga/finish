	    ...
	    <table width="610px" border="0" align="center">
          <tr>
            <td>
				...
				<tr><td ><h4>Numeracion</h4></td><td ><h4>Nombre del laboratorio</h4>
				</tr>
				</thead>
				<?php
			  if(isset($_REQUEST['buscar1'])or isset($_REQUEST['criterios'])){
			    ...
			    if ($buscar1!="" or $criterios=='Todos'  ) {
					   ...
					   if ($criterios=='Todos') {
							$_pagi_sql=" select * from laboratorio"; 
					   }
						...
						//if (mysql_num_rows($result1)>=1) {
							...
							{
								echo '<tr><td data-label="Curp"><a href="modificaclientes.php?aplicar5='.$row["Id_Laboratorio"].'">'.$row["Id_Laboratorio"].'</a></td>
								<td data-label="Nombre">'.$row["Nombre_Laboratorio"].'</td>
